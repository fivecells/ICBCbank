...
BrowserCompatible.prototype.setFormAppendTimestamp = function(){
	if(document.readyState=="complete"){
		...
		for(var i=0;i<forms.length;i++){
			...
			if((formTarget==null||formTarget=="_self")&&forms[i].outerHTML.indexOf("randomIdAppendToFormForMacSafari")==-1){
...
BrowserCompatible.prototype.alertHistoryTip = function(){
	...
	if(document.getElementById("alerthistorytip-outbackground")==null){
		...
		html = html + "<div id='alerthistorytip-inner-content-left' style='width:121px;height:135px;float:left;background-image:url(../images/back.gif);'>";
		html=html+"</div>";
		...
		html=html+"<a href='javascript:pebankBrowserCompatible.hideTip()'><img src='../images/bt44.gif' border='0'/></a>";
		html=html+"</div>";
		html=html+"</div>";
		html=html+"</div>";
		html=html+"</div>";
...
function pulginHasInstalled() {
	...
	if(pebankBrowserCompatible.isFirefox()){
		...
		//typeof(navigator.mimeTypes['application/x-icbc-plugin-chrome-npxxin-store'])!="undefined" &&
